...
@section('title', 'Data Return Jual')
@section('page-name', 'Data Return Jual')
@section('content')
...
<div class="app-content pt-3 p-md-3 p-lg-4">
    <div class="container-xl">
        <div class="row g-3 mb-4 align-items-center justify-content-between">
            <div class="col-md-8">
                <h1 class="app-page-title mb-0">Data Return Jual</h1>
            </div>
            <div class="col-auto">
        ...
        <div class="tab-content" id="orders-table-tab-content">
            <div class="tab-pane fade show active" id="orders-all" role="tabpanel" aria-labelledby="orders-all-tab">
                            ...
                            <table id="dataTable2" class="table table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="cell sortable">Kode Pesanan<i class="fas fa-sort sort-icon"></i></th>
                                        ...
                                        <th class="cell sortable">Price<i class="fas fa-sort sort-icon"></i></th>
                                ...
                                <tbody>
                                    @php
                                        $no = $returnjual->firstItem(); // Nomor indeks dimulai dari halaman saat ini
                                    @endphp
                                    @foreach ($returnjual as $item)
                                        <tr>
                                            ...
                                            <td class="cell"><span class="truncate">{{ $item->quantity }}</span></td>
                                            <td class="cell"><span class="truncate">Rp. {{ number_format($item->price) }}</span></td>
                                            <td class="cell"><span class="truncate">{{ $item->created_at }}</span></td>
                ...
                <div class="row mb-4">
                    <div class="col-auto">
                        <!-- Showing information -->
                        Showing {{ $returnjual->firstItem() }} to {{ $returnjual->lastItem() }} of
                        {{ $returnjual->total() }} entries
                    </div>
                    <div class="col-auto ms-auto">
                        ...
                        <nav class="app-pagination">
                            <ul class="pagination justify-content-center">
                                ...
                                @if ($returnjual->onFirstPage())
                                    <li class="page-item disabled">
                                        <span class="page-link" aria-label="Previous">&laquo;</span>
                                    </li>
                                @else
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $returnjual->previousPageUrl() }}"
                                            aria-label="Previous">&laquo;</a>
                                    </li>
                                @endif

                                <!-- Tombol Halaman -->
                                @for ($i = 1; $i <= $returnjual->lastPage(); $i++)
                                    <li class="page-item {{ $i == $returnjual->currentPage() ? 'active' : '' }}">
                                        <a class="page-link"
                                            href="{{ $returnjual->url($i) }}">{{ $i }}</a>
                                    </li>
                                @endfor

                                <!-- Tombol Halaman Berikutnya -->
                                @if ($returnjual->hasMorePages())
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $returnjual->nextPageUrl() }}"
